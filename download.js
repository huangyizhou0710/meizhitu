...
module.exports = function download(imageName, type = 'beauty', params = { year: '2024', month: '01', categoryid: '108366' }) {
  const downloadUrl = {
    'beauty': `https://image.baidu.com/search/down?url=https://wx1.sinaimg.cn/mw1024/${imageName}`,
    'photo': `https://p.meizitu.net/${params.year}/${params.month}/${imageName}`
  }
  const localPath = {
  ...
  .then(response => {
    // 创建可写流并将数据写入本地文件
    response.data.pipe(fs.createWriteStream(localPath[type] + '/' + imageName ));
